---
title: 常見問題與故障排查 FAQ
description: EpoCanvas Docs 本地執行報錯、文件不顯示、提示框渲染異常、搜尋失效與 Cloudflare Pages 部署的排查清單。
---

在使用、撰寫或部署 **EpoCanvas Docs** 的過程中遇到異常時，先在這裡對號入座。問題按「本地啟動 → 文件撰寫 → 搜尋 → 部署」的順序排列，每個問題都給出原因和驗證過的解決辦法。

---

## 一、本地啟動與安裝問題

### Q1：執行 `pnpm run dev` 提示連接埠 4321 被占用

- **原因**：之前啟動的開發伺服器沒有完全退出，或其他程式占用了 4321 連接埠。
- **解決辦法**：換一個連接埠啟動：
...
- **解決辦法**：刪除內文中的 `#` 標題，小節從 `##` 開始。完整規則見[渲染規則詳解](/canvas/rendering/#标题规则)。
...
### Q9：本地 `pnpm dev` 除錯時，全域搜尋搜不到剛寫的新文章
...
- **原因**：部分輸入法、剪貼簿工具或截圖軟體占用了 `Ctrl+K` / `Cmd+K` 快捷鍵。
...
向 GitHub 推送前，執行以下指令做一次完整自檢（型別檢查 + 全量建置）：

```bash
pnpm exec astro check && pnpm run build
```

`astro check` 輸出 `0 errors`、建置以 `Complete!` 結束時，說明文件沒有語法錯誤，可以放心提交。儲存庫的 CI（`build.yml`）會在推送後執行同樣的建置，本地先通過可以避免 CI 失敗。

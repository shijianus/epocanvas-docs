...
4. **Multilinguagem apenas pela metade**: muitos sites de documentação dizem suportar vários idiomas, mas na prática só traduzem os botões de navegação — ao abrir o corpo do artigo, o texto continua no idioma original; noutros, quando falta uma tradução, a página devolve diretamente um 404 e o leitor tem de editar o URL à mão para encontrar o conteúdo.
...
- **Barra de índice à direita**: extrai automaticamente os títulos `h2` e `h3` do artigo para gerar o índice da página, destaca a posição de leitura atual à medida que se faz scroll, e salta suavemente para qualquer subsecção com um clique.

### 2. Pesquisa em dois modos: dentro da página + em todo o site

- **Localizar na página atual**: escreva palavras-chave diretamente na caixa de pesquisa do topo e todos os textos correspondentes na página são imediatamente destacados, com um contador de progresso como `3/9`; prima Enter para saltar de ocorrência em ocorrência.
...
- Clique no botão de idioma no canto superior direito para saltar para a versão do mesmo artigo nesse idioma; o URL tem o prefixo do idioma (por exemplo, `/en/canvas/`), pelo que pode ser guardado nos favoritos ou partilhado diretamente com colegas que usem outros idiomas.
...
- Suporta sintaxes extensas do Markdown de uso comum, como tabelas GFM, listas de tarefas e texto riscado; as regras completas estão em [Regras de renderização em detalhes](/canvas/rendering/).
...
| **Forma de implantação** | Envio direto para o Cloudflare Pages | S3 / Vercel / GitHub | GitHub Pages | Alojamento privado da plataforma |
| **Grau de controlo próprio** | 100% código aberto, com o código-fonte totalmente nas suas mãos | 100% código aberto | 100% código aberto | Código fechado, com muitas funcionalidades pagas |
...
| **Ambiente de execução** | Node.js 18.20.8 / 20.3+ / 22+, pnpm 10 | Desenvolvimento e compilação locais; `pnpm run deploy` exige Node.js 22 ou mais recente por causa do wrangler |
...
│   └── pages/404.astro        # página 404 própria, mensagem conforme o idioma do browser
├── scripts/                   # invalidação de cache, verificações de i18n / imagens / âncoras e capturas

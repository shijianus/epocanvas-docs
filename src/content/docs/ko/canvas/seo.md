...
`public/`으로 참조되는 정적 리소스는 배포 시 CDN으로 배포됩니다. 빌드 도구 체인에는 sharp 이미지 처리 모듈이 내장되어 있어 이후 빌드 시점 이미지 최적화를 도입할 수 있는 기반이 마련되어 있습니다. 현재 규범은 스크린샷 너비를 1440픽셀 정도로 제한하고 다이어그램은 SVG를 우선 사용하도록 요구하여, 근원에서 이미지 용량을 통제합니다.

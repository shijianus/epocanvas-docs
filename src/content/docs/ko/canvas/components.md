...
| 컴포넌트 파일 | 규모 | 역할 |
| :--- | :--- | :--- |
| `Header.astro` | 약 646줄 | 상단 바의 전체 내용: 로고, 검색창, 메인 내비게이션, 버전 배지, 언어 전환, 테마 전환, GitHub 및 Telegram 진입점 |
...
| `TableOfContents.astro` | 약 64줄 | "이 페이지의 목차" 제목, 아이콘과 목차 목록, 페이지 자체 제목은 필터링 |
...
- **UI 문구**(검색창 플레이스홀더, "이 페이지의 목차" 제목, 테마 전환 안내 등)는 각 컴포넌트가 `getTranslation(key, lang)`을 호출하여 빌드 시점에 해당 언어로 직접 출력하며, 페이지 안에 런타임 치환 스크립트는 없습니다.
...
오른쪽 목차 열의 너비는 `@media (min-width: 72rem)`에서 `20rem`으로 고정되어 있고(초광폭 스크린 `90rem` 이상에서는 `21rem`), 본문 영역의 최대 너비는 여기서 오른쪽 열 너비를 뺀 값입니다. 오른쪽 열 너비를 조정하려면 이 파일 하나만 수정하면 되며, 다른 스타일시트에 흩어져서 오버라이드하지 마세요.
...
- 상단 바 오른쪽의 GitHub와 Telegram 아이콘은 모두 `src/components/starlight/Header.astro`에 직접 지정되어 있으므로 주소를 바꾸려면 이 파일을 고치면 됩니다. Telegram은 `@epocanvas` 계정 페이지를 가리키며, 툴팁 문구는 낱말감 `social.telegram`입니다. `astro.config.mjs`의 `social`이 제어하는 것은 사이드바 아래쪽의 아이콘 묶이며, 두 곳은 서로 영향을 주지 않습니다.

### Search: 듀얼 모드 검색

하나의 컴포넌트에 두 가지 검색이 구현되어 있습니다(자세한 내용은 [전체 텍스트 검색 및 단축키](/canvas/search-engine/) 참조):
